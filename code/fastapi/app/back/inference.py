...
class InferenceOnDevice(Inference):
    def __init__(self, images, exam_info, coco, detector):
        self.images = images
        self.coco = coco
        self.detector = detector
    ...
    def get_predict(self, img, box_threshold=0.3):
        ...
        Returns:
            List: [np.array(left,top,right,bottom,confidence,label) .... ]
            list안의 값은 np.array 형식으로 박스정보와 label 값이 float type으로 정의됨
        """
...
if __name__ == "__main__":
    ...
    detector = Detector(
        model_path="/opt/ml/input/data/models/on-device-model",
        device_name="cpu",
    )

    img_path = "/opt/ml/input/data/infer_test"
    img_paths = [os.path.join(img_path, img) for img in os.listdir(img_path)]
    images_np = [cv2.imread(img) for img in sorted(img_paths)]
